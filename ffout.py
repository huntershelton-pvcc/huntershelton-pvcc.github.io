...
# output file 
    ...
    for i in range(num_emps):
        data1 = "\n" + emp[i] + tab + job[i] + format(gross_pay[i], currency)
        data2 = format(fed_tax[i], currency) + tab+tab + format(state_tax[i], currency) + tab+tab
        data3 = format(soc_sec[i], currency) + tab+tab + format(medicare[i], currency) + tab+tab
        data4 = format(ret401k[i], currency) + tab+tab + format(net_pay[i], currency)
